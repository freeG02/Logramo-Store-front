...
// Sends the Logramo welcome email via Resend whenever a new row appears
// in public.subscribers. Trigger via a Supabase Database Webhook on INSERT.
//
// ENV required:
...
function smartFirstName(email: string, name?: string | null): string {
  if (name && name.trim()) {
  ...
  let candidate = parts[0].replace(/\d+/g, "");
...
// Inline Logramo wordmark SVG (used in header + footer) — color via fill="currentColor"
...
const FB_ICON = `<svg width="38" height="38" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" style="display:inline-block;vertical-align:middle;color:#111A17;fill:currentColor;padding:8px;box-sizing:border-box;"><path d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z"/></svg>`;
const YT_ICON = `<svg width="38" height="38" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" style="display:inline-block;vertical-align:middle;color:#111A17;fill:currentColor;padding:9px;box-sizing:border-box;"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>`;
const PIN_ICON = `<svg width="38" height="38" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" style="display:inline-block;vertical-align:middle;color:#111A17;fill:currentColor;padding:8px;box-sizing:border-box;"><path d="M12 0C5.373 0 0 5.372 0 12c0 5.084 3.163 9.426 7.627 11.174-.105-.949-.2-2.405.042-3.441.218-.937 1.407-5.965 1.407-5.965s-.359-.719-.359-1.782c0-1.668.967-2.914 2.171-2.914 1.023 0 1.518.769 1.518 1.69 0 1.029-.655 2.568-.994 3.995-.283 1.194.599 2.169 1.777 2.169 2.133 0 3.772-2.249 3.772-5.495 0-2.873-2.064-4.882-5.012-4.882-3.414 0-5.418 2.561-5.418 5.207 0 1.031.397 2.138.893 2.738.098.119.112.224.083.345l-.333 1.36c-.053.22-.174.267-.402.161-1.499-.698-2.436-2.889-2.436-4.649 0-3.785 2.75-7.262 7.929-7.262 4.163 0 7.398 2.967 7.398 6.931 0 4.136-2.607 7.464-6.227 7.464-1.216 0-2.359-.631-2.75-1.378l-.748 2.853c-.271 1.043-1.002 2.35-1.492 3.146C9.57 23.812 10.763 24 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0z"/></svg>`;
...
function buildHtml(firstName: string): string {
  // Personalized greeting line for the hero headline
  const heroLine = firstName
    ? `${esc(firstName)},<br>ahora<br>eres uno<br>de <span style="color:#C55932;font-style:italic;">nosotros.</span>`
    : `Ahora<br>eres uno<br>de <span style="color:#C55932;font-style:italic;">nosotros.</span>`;

  return `<!DOCTYPE html>
<html lang="es"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width, initial-scale=1.0"/><title>Bienvenida a Logramo</title></head>
...
<table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" align="center" style="width:600px;max-width:100%;background:#FEFAE8;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;color:#111A17;">
    ...
    <td style="padding:24px 32px 18px;border-bottom:1px solid rgba(17,26,23,.12);">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
        <tr>
          ...
          <td align="right" valign="middle" style="font-family:'Helvetica Neue',Arial,sans-serif;font-weight:700;font-size:10px;letter-spacing:.18em;text-transform:uppercase;color:#3C4824;">Bienvenida · ${monthYear()}</td>
        </tr>
      </table>
    </td>
  </tr>
  ...
  <tr>
    <td style="padding:54px 32px 44px;">
      <div style="font-family:'Helvetica Neue',Arial,sans-serif;font-weight:900;font-size:11px;letter-spacing:.18em;text-transform:uppercase;color:#C55932;margin-bottom:18px;">Estás dentro</div>
      ...
      <p style="margin:28px 0 0;font-family:'Helvetica Neue',Arial,sans-serif;font-size:17px;line-height:1.5;color:#3C4824;max-width:440px;">
        Dueños imperfectos criando perros felices. Sin manuales aburridos, sin juicios — solo lo que de verdad te sirve, como te lo contaría un amigo que ya pasó por el caos.
      </p>
    </td>
  </tr>
  ...
  <tr>
    <td style="padding:0;">
      <img src="https://images.unsplash.com/photo-1583511655857-d19b40a7a54e?w=1200&q=85&fit=crop" alt="" width="600" style="display:block;width:100%;height:auto;border:0;">
    </td>
  </tr>

  <tr>
    <td style="background:#3C4824;padding:48px 32px;color:#FEFAE8;">
      <div style="font-family:'Helvetica Neue',Arial,sans-serif;font-weight:900;font-size:11px;letter-spacing:.18em;text-transform:uppercase;color:#F6D055;margin-bottom:18px;">Lo que viene</div>
      <h2 style="margin:0 0 18px;font-family:'Arial Black','Helvetica Neue',Arial,sans-serif;font-weight:900;font-size:34px;line-height:1;letter-spacing:-.02em;text-transform:uppercase;color:#FEFAE8;">Avisos cuando<br>haga falta. Nada más.</h2>
      <p style="margin:0 0 32px;font-family:'Helvetica Neue',Arial,sans-serif;font-size:16px;line-height:1.55;color:rgba(254,250,232,.78);">
        Te escribiremos cuando publiquemos una guía nueva, un artículo que sirva, o algo que <em>de verdad</em> te ayude. Cero spam, cero relleno.
      </p>
      <table role="presentation" cellpadding="0" cellspacing="0" border="0">
        <tr>
          <td style="background:#F6D055;border:2px solid #111A17;border-radius:12px;box-shadow:4px 4px 0 #111A17;">
            <a href="${SITE_URL}/biblioteca.html" style="display:inline-block;padding:14px 26px;font-family:'Helvetica Neue',Arial,sans-serif;font-weight:900;font-size:13px;letter-spacing:.1em;text-transform:uppercase;color:#111A17;text-decoration:none;">Ver la biblioteca &nbsp;→</a>
          </td>
        </tr>
      </table>
    </td>
  ...
  <tr>
    <td style="background:#F8F3D9;padding:40px 32px;">
      <div style="font-family:'Helvetica Neue',Arial,sans-serif;font-weight:900;font-size:11px;letter-spacing:.18em;text-transform:uppercase;color:#C55932;margin-bottom:20px;">Mientras tanto</div>
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
        <tr><td style="padding:14px 0;border-bottom:1px solid rgba(17,26,23,.1);">
          <a href="${SITE_URL}/biblioteca.html" style="text-decoration:none;color:inherit;display:block;">
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
              ...
              <td>
                <div style="font-family:'Arial Black','Helvetica Neue',Arial,sans-serif;font-weight:900;font-size:18px;letter-spacing:-.01em;color:#111A17;">Explora la biblioteca &nbsp;→</div>
                <div style="font-family:'Helvetica Neue',Arial,sans-serif;font-size:14px;color:#3C4824;margin-top:4px;">Guías de educación, conducta, salud — empieza por la que más te urja.</div>
              </td>
            </tr></table>
          </a>
        </td></tr>
        <tr><td style="padding:14px 0;border-bottom:1px solid rgba(17,26,23,.1);">
          <a href="${SITE_URL}/blog.html" style="text-decoration:none;color:inherit;display:block;">
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
              ...
              <td>
                <div style="font-family:'Arial Black','Helvetica Neue',Arial,sans-serif;font-weight:900;font-size:18px;letter-spacing:-.01em;color:#111A17;">Lee el blog &nbsp;→</div>
                <div style="font-family:'Helvetica Neue',Arial,sans-serif;font-size:14px;color:#3C4824;margin-top:4px;">Consejos cortos, historias reales y algún que otro fail canino épico.</div>
              </td>
            </tr></table>
          </a>
        </td></tr>
        <tr><td style="padding:14px 0;">
          <a href="https://www.facebook.com/profile.php?id=61590239832119" style="text-decoration:none;color:inherit;display:block;">
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
              ...
              <td>
                <div style="font-family:'Arial Black','Helvetica Neue',Arial,sans-serif;font-weight:900;font-size:18px;letter-spacing:-.01em;color:#111A17;">Síguenos &nbsp;→</div>
                <div style="font-family:'Helvetica Neue',Arial,sans-serif;font-size:14px;color:#3C4824;margin-top:4px;">En Facebook, YouTube y Pinterest — para no perderte nada.</div>
              </td>
            </tr></table>
          </a>
        </td></tr>
...
serve(async (req: Request) => {
  // Health check
  if (req.method === "GET") {
    return new Response("Logramo send-welcome OK", { status: 200 });
  }
  if (req.method !== "POST") {
    return new Response("Method not allowed", { status: 405 });
  }

  // Optional shared secret (set via WEBHOOK_SECRET env, then add the same
  // value as a custom header in Supabase Database Webhook settings)
  if (WEBHOOK_SECRET) {
